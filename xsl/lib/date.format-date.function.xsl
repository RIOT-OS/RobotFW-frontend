...
<!--
   This is an implementation on exslt date:format-date by Jeni Tennison
   See http://exslt.org/date/functions/format-date/index.html

   - There are changes to the original code because it can't handle milliseconds
     properly. Search for "Edited" in order to find the changes.

   - The import statement needs to be commented
-->
...
<func:function name="date:_format-date">
   ...
   <!-- Edited (added) -->
   <xsl:variable name="format-has-millisecons" select="substring($format, string-length($format), 1) = 'S'" />
   <!-- End: Edited -->
                 ...
                 <xsl:choose>
                    ...
                    <xsl:when test="$char = 's'">
                       <xsl:choose>
                          ...
                          <xsl:when test="$format-has-millisecons = 'true'">
                             <xsl:value-of select="format-number(substring-before($second, '.'), $padding)" />
                          </xsl:when>
                          <xsl:otherwise>
                    ...
                    <xsl:when test="$char = 'S'">
                       <xsl:choose>
                          <xsl:when test="string($second) = 'NaN'" />
                          <xsl:otherwise>
                             <xsl:value-of select="substring-after(format-number(concat('.', substring-after($second, '.')), concat('.', $padding)), '.')" />
                          </xsl:otherwise>
                       </xsl:choose>
                    </xsl:when>
                    <!-- End: Edited -->
